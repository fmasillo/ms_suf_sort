...
bool compareSuf(const Suf& a, const Suf& b){
   ...
   //uint64_t counter;
   //for(counter = 0; *(_sx + a.idx + docBoundaries[a.doc - 1] + a.len + counter) == *(_sx + b.idx + docBoundaries[b.doc - 1] + b.len + counter); counter++){}
   //std::cerr << "solved after " << counter << "\n";
   return *(_sx + a.idx + docBoundaries[a.doc - 1] + a.len + counter) < *(_sx + b.idx + docBoundaries[b.doc - 1] + b.len + counter);
}
...
bool compareHeads(const Match &a, const Match &b){
   return a.start < b.start;
}

std::vector<Match>::iterator getHead(Suf a){
...
bool compareSufAndHeads(Match head, Suf suf){
   return suf.idx + docBoundaries[suf.doc - 1] < head.start;
}
...
int lzFactorize(char *fileToParse, int seqno, char* outputfilename, bool v) {
    ...
    //for(size_t i = 1; i < headBoundaries.size(); i++) {headBoundaries[i] = headBoundaries[i] + headBoundaries[i-1];}
    //MSGSA.pop_back();
    ...
    if(verbose) std::cerr << "Printing headBoundaries" << "\n";
    ...
    if(verbose) std::cerr << "Printing unsorted MSGSA\n";
    ...
    for(size_t i = 1; i < _n; i++){
       std::cerr << "Sorting bucket from " << prefSumBucketLengthsCopy[i-1] << " to " << prefSumBucketLengthsCopy[i] << "\n";
       ...
       [](const Suf a, const Suf b){
         //bool tmpVerbose = false;
         //if(((a.idx == 13303 && a.doc == 2572) && (b.idx == 13305 && b.doc == 3022)) || ((b.idx == 13303 && b.doc == 2572) && (a.idx == 13305 && a.doc == 3022))) tmpVerbose = true;
         /*if(a.len != b.len){
            //if(tmpVerbose) std::cerr << "suf_a " << a.idx << " " << a.doc << " " << a.len << ": " << _sx + a.idx + docBoundaries[a.doc - 1];
            //if(tmpVerbose) std::cerr << "suf_b " << b.idx << " " << b.doc << " " << b.len << ": " << _sx + b.idx + docBoundaries[b.doc - 1];
            //diffLen++;
            uint32_t nextMM = std::min(a.len, b.len);
            return *(_sx + a.idx + docBoundaries[a.doc - 1] + nextMM) < *(_sx + b.idx + docBoundaries[b.doc - 1] + nextMM);
         }
         if (*(_sx + a.idx + docBoundaries[a.doc - 1] + a.len) != *(_sx + b.idx + docBoundaries[b.doc - 1] + b.len)){
            //if(tmpVerbose) std::cerr << "suf_a " << a.idx << " " << a.doc << " " << a.len << ": " << _sx + a.idx + docBoundaries[a.doc - 1];
            //if(tmpVerbose) std::cerr << "suf_b " << b.idx << " " << b.doc << " " << b.len << ": " << _sx + b.idx + docBoundaries[b.doc - 1];
            return *(_sx + a.idx + docBoundaries[a.doc - 1] + a.len) < *(_sx + b.idx + docBoundaries[b.doc - 1] + b.len);
         }*/
         uint32_t nextMM = std::min(a.len, b.len);
         ...
         if((headA->len == 0) & (headB->len == 0)){
            return a.doc < b.doc;
         }
         //if(tmpVerbose) std::cerr << "suf_a " << a.idx << " " << a.doc << ": " << _sx + a.idx + docBoundaries[a.doc - 1];
         //if(tmpVerbose) std::cerr << "suf_b " << b.idx << " " << b.doc << ": " << _sx + b.idx + docBoundaries[b.doc - 1];
         //if(tmpVerbose) std::cerr << "head_a " << headA->start << "," << headA->pos << "," << headA->len << "\n";
         //if(tmpVerbose) std::cerr << "head_b " << headB->start << "," << headB->pos << "," << headB->len << "\n";
         uint64_t ndocPlusA = 1*(headA->len == 0);
         uint64_t ndocPlusB = 1*(headB->len == 0);
         //if(headA->len == 0) ndocPlusA++;
         //if(headB->len == 0) ndocPlusB++;
         headA++;
         headB++;
         //if(tmpVerbose) std::cerr << "after ++ head_a " << headA->start << "," << headA->pos << "," << headA->len << "\n";
         //if(tmpVerbose) std::cerr << "after ++ head_b " << headB->start << "," << headB->pos << "," << headB->len << "\n";
         while(headA->len == headB->len){
            if((headA->len == 0) & (headB->len == 0)){//if(tmpVerbose) std::cerr << "doc++\n"; 
               return a.doc < b.doc;
               headA++; headB++; ndocPlusA++; ndocPlusB++; 
               //if(a.doc - 1 + ndocPlusA >= docBoundaries.size()) return true; 
               //if(b.doc - 1 + ndocPlusB >= docBoundaries.size()) return false; 
               continue;
            } //just go one head after and check len in outer while
            if(headA->pos != headB->pos){return _ISA[headA->pos] < _ISA[headB->pos];}
            if(*(_sx + headA->start + docBoundaries[a.doc - 1 + ndocPlusA] + headA->len) != *(_sx + headB->start + docBoundaries[b.doc - 1 + ndocPlusB] + headB->len)){
               //if(tmpVerbose) std::cerr << _sx + headA->start + docBoundaries[a.doc - 1 + ndocPlusA] + headA->len;
               //if(tmpVerbose) std::cerr << _sx + headB->start + docBoundaries[b.doc - 1 + ndocPlusB] + headB->len;
            ...
            //uint32_t prevLenA = headA->len;
            //uint32_t prevLenB = headB->len;
            //if(tmpVerbose) std::cerr << "nextStartA " << nextStartA << "; nextStartB " << nextStartB << "\n";
            while(headA->start + headA->len <= nextStartA){
               headA++; 
               //if(tmpVerbose) std::cerr << "in inside whileA " << headA->start << "," << headA->len <<"\n"; 
               //if(!(headA->len < (headA-1)->len)) break;
               //if(headA->len == 0) break;
               //prevLenA = headA->len;
            }
            while(headB->start + headB->len <= nextStartB){
               headB++; 
               //if(tmpVerbose) std::cerr << "in inside whileB " << headB->start << "," << headB->len <<"\n"; 
               //if(!(headB->len < (headB-1)->len)) break; 
               //if(headB->len == 0) break;
               //prevLenB = headB->len;
            }
            if((headA->start - nextStartA) != (headB->start - nextStartB)){
               //uint32_t nextMM = std::min(headA->start - nextStartA, headB->start - nextStartB);
               //return *(_sx + docBoundaries[a.doc - 1 + ndocPlusA] + nextMM) < *(_sx + docBoundaries[b.doc - 1 + ndocPlusB] + nextMM);
               return _ISA[headA->pos - (headA->start - nextStartA)] < _ISA[headB->pos - (headB->start - nextStartB)];
            }
            //if(tmpVerbose) std::cerr << "After aug headA " << headA->start << "," << headA->pos << "," << headA->len << "\n";
            //if(tmpVerbose) std::cerr << "After aug headB " << headB->start << "," << headB->pos << "," << headB->len << "\n";
            //if(tmpVerbose) std::cerr << *(_sx + headA->start + docBoundaries[a.doc - 1 + ndocPlusA] + headA->len);
            //if(tmpVerbose) std::cerr << *(_sx + headB->start + docBoundaries[b.doc - 1 + ndocPlusB] + headB->len);
         }
         if(headA->pos != headB->pos){return _ISA[headA->pos] < _ISA[headB->pos];}
         //if(tmpVerbose) std::cerr << "out of while\n";
         nextMM = std::min(headA->len, headB->len);
         //if(nextMM == 0) nextMM++;
         //if(tmpVerbose) std::cerr << *(_sx + headA->start + docBoundaries[a.doc - 1 + ndocPlusA] + nextMM);
         //if(tmpVerbose) std::cerr << *(_sx + headB->start + docBoundaries[b.doc - 1 + ndocPlusB] + nextMM);
         return *(_sx + headA->start + docBoundaries[a.doc - 1 + ndocPlusA] + nextMM) < *(_sx + headB->start + docBoundaries[b.doc - 1 + ndocPlusB] + nextMM);
       });
    }
    ...
    [](const Suf a, const Suf b){
      //bool tmpVerbose = false;
      //if(((a.idx == 25892 && a.doc == 33) && (b.idx == 1713 && b.doc == 23)) || ((b.idx == 25892 && b.doc == 33) && (a.idx == 1713 && a.doc == 23))) tmpVerbose = true;
      /*if(a.len != b.len){
         //diffLen++;
         uint32_t nextMM = std::min(a.len, b.len);
         return *(_sx + a.idx + docBoundaries[a.doc - 1] + nextMM) < *(_sx + b.idx + docBoundaries[b.doc - 1] + nextMM);
      ...
      */
      ...
      //if(tmpVerbose) std::cerr << "head_a " << headA->start << "," << headA->pos << "," << headA->len << "\n";
      //if(tmpVerbose) std::cerr << "head_b " << headB->start << "," << headB->pos << "," << headB->len << "\n";
      uint64_t ndocPlusA = 0;
      uint64_t ndocPlusB = 0;
      if(headA->len == 0) ndocPlusA++;
      if(headB->len == 0) ndocPlusB++;
      headA++;
      headB++;
      ...
      //if(tmpVerbose) std::cerr << "after ++ head_b " << headB->start << "," << headB->pos << "," << headB->len << "\n";
      while(headA->len == headB->len){
         if(headA->len == 0){//if(tmpVerbose) std::cerr << "doc++\n"; 
            return a.doc < b.doc;
            headA++; headB++; ndocPlusA++; ndocPlusB++; 
            //if(a.doc - 1 + ndocPlusA >= docBoundaries.size()) return true; 
            //if(b.doc - 1 + ndocPlusB >= docBoundaries.size()) return false; 
    ...
    std::cerr << "meanCounter " << sumCounter/(denCounter+0.0000001) << "\n";
