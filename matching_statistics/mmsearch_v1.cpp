...
struct Suf{
   ...
   uint32_t len;
   Suf(){}
   Suf(uint32_t i, uint32_t d, uint32_t p, uint32_t l){
      idx = i; doc = d; pos = p, len = l;
   }
};
...
int lzFactorize(char *fileToParse, int seqno, char* outputfilename, bool verbose) {
    ...
    while(i < _sn) {
        ...
        //std::cerr << "\n";
        MSGSA.push_back(Suf(i_current_doc, ndoc, _ISA[pos], len));
        i_current_doc++;
        ...
        if(len == 1 || pos == _n){
           ...
           if(leftB == rightB && len > _maxLCP){
              //std::cout << "Case 1: leftB == rightB && len > _maxLCP" << "\n";
              //std::cout << "Before: leftB " << leftB << ", rightB " << rightB << "\n";
              leftB = rightB = _ISA[_SA[leftB]+1];
              //std::cout << "After: leftB " << leftB << ", rightB " << rightB << "\n";
              _numberOfSingleMatches++;
           }else{
              //bool a = leftB == rightB;
              //bool b = len > _maxLCP;
              //std::cout << "Case 2: leftB == rightB: " << a << "; len > _maxLCP" << b << "\n";
              //std::cout << "Before contractLeft: leftB " << leftB << ", rightB " << rightB << "\n";
              std::pair<int,int> interval = contractLeft(leftB,rightB,len);
              leftB = interval.first;
              rightB = interval.second;
              //std::cout << "After contractLeft: leftB " << leftB << ", rightB " << rightB << "\n";
           }
        }
    }
    MSGSA.pop_back();
    ...
    //std::sort(MSGSA.begin(), MSGSA.end(), comparePos);
    t1 = std::chrono::high_resolution_clock::now();
    std::sort(MSGSA.begin(), MSGSA.end(), compareSuf);
    ...
    std::cerr << "times it had to compare only the positions " << diffPos << "\n";
    delete[] sx;
    //delete[] GSA;
...
void computeLZFactorAt(filelength_type i, filelength_type *pos, filelength_type *len, int64_t & leftB, int64_t & rightB, int64_t & maxMatch, bool & isSmallerThanMaxMatch, unsigned char &mismatchingSymbol) {
    ...
    while (j < _sn) { //scans the string from j onwards until a maximal prefix is found between _x and _sx
        ...
        if (nlb == nrb) { 
            ...
            //std::cout << "nlb after binary search " << nlb << "\n";
            //std::cerr << "nlb: " << nlb << "\n";
            if (nlb < 0) {
